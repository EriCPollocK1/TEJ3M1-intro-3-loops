...
this is running if SW2 or SW3 are pressed and only once.
...
it is only running when PWMperiod is 0, and it will run however many times it needs to.

 * 3. What condition is being evaluated by the if statement inside the loop?
 *    What happens when the if condition is true?
if the period and the brightness match then LED4 will be on
...
if we counted every new press it would be less than 1% but since we are not counting every new press 
but instead we are counting thousands of times per milisecond each "press" is probably 15%-30%
...
yes the LED is still lit, therefore the value of PWMperiod must be atleast 1
...
yes you would be able to as it doesn't matter for outside the for and is defined inside the from
